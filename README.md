...
- if at any given configuration of the robot, it collides with an obstacle or voilates joint constraints, than that configuration is marked as occupied in the configuration space. 
- If no such collision or voilation occurs than that configuration is marked as 
...
### Motion Planning
The configuration space is computed as a standard occupancy grid with 1 representing occupied space and 0 representing free space. Motion planning algorithms can be used to find a path between any starting pose and goal pose of the robotic manipulator. We can use the configuration space where the poses are simply points and we need to find a path connecting those points while avoiding constraints and obstacles. Such approaches must, however, account for the warped nature of the configuration space (0 degree joint angle is the same as 2pi joint angle)

### Pre-Computed Motion plan
We computed an example path between an starting and end pose by using the navigation with polytopes tool box [2]. Currently it is not integrated with this repository. 
